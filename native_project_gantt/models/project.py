...
class ProjectTask(models.Model):
    ...
    proj_employee=fields.Many2one('hr.employee','Employee')
    proj_role=fields.Char('Role')
    log_in=fields.Boolean('Log In ?')
    shift_sent=fields.Boolean('IS the Shift Sent')
    allocated_hrs=fields.Float('Allocated Hours')
    ...
    def _onchange_end_date(self):
        print("jkkkkkkkkkkkkkkkkkkkk")
        if self.start_datetime and self.end_datetime:
            time_pro=self.end_datetime - self.start_datetime
            self.allocated_hrs = float(time_pro.days) * 24 + (float(time_pro.seconds) / 3600)
